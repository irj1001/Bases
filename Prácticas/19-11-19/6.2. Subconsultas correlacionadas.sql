...
/*Modelo de explicación para el examen:
*Devuelve la población de la oficina cuyo objetivo es mayor que las cuotas de sus vendedores
*/
SELECT poblacion FROM oficinas 
WHERE objetivo > (	SELECT SUM(cuota)
			FROM vendedores
...
**************************************************/
/*Ejercicio 15
Oficinas donde el objetivo es mayor que la suma de los salarios de dichas oficina
sal+comision*ventas/100
*/

select n_oficina from oficinas
where objetivo>(select sum(sal+comision*ventas/100)
...
--Oficinas con numero de empleados mayor que la media

select n_oficina, count(idVendedor) from oficinas join vendedores on(n_oficina=oficina)
group by n_oficina
having (count(idVendedor))>(select count(idVendedor)/count ( distinct n_oficina) from vendedores right join oficinas on(n_oficina=oficina));


select oficina, count (idVendedor) from vendedores
group by oficina
having count(idVendedor)>(select avg(media)
	from(select oficina, count(idVendedor) as media
		from vendedores right join oficinas on(oficina=n_oficina)
		group by oficina)recuentos);



select n_oficina, idVendedor 
from oficinas left join vendedores on (n_oficina=oficina);